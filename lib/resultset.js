        ...
        if (err) {
            ...
            rsmd.getColumnCount(function (err, colcount) {
                for (var i = 1; i < colcount; i++) {
                    colsmetadata[rsmd._rsmd.getColumnLabelSync(i)] = "string";
                }
                var result = [];
                var rowIter = self._rs.nextSync();
                while (rowIter) {
                    var myModel = {};
                    for (var r in colsmetadata) {
                        myModel[r] = self._rs.getStringSync(r);
                    }
                    result.push(myModel);
                    rowIter = self._rs.nextSync();
                }
